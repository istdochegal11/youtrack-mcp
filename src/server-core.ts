...
export class YouTrackMCPServer {
   ...
   */
  private resolveProjectId(providedProjectId?: string, allowOverride: boolean = false): string | undefined {
    const config = this.config.get();
    const configuredProjectId = config.defaultProjectId;

    if (configuredProjectId && providedProjectId && !allowOverride) {
      throw new Error("Project ID override ist not allowed.");
    }


    return providedProjectId || configuredProjectId;
  }
